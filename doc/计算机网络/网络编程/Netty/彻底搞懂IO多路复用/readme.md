...
1. 操作系统提供的 read 函数阻塞在了两个阶段（网卡->内核缓存区，内核缓存区->用户缓存区）

缺点：所以，如果这个连接的客户端一直不发数据，那么服务端线程将会一直阻塞在 read 函数上不返回，也无法接受其他客户端连接。

2. 优化 read 函数为非阻塞并使用多线程去处理 read 函数
（1）操作系统提供了非阻塞的 read 函数，只需要在调用 read 前，将文件描述符设置为非阻塞即可，其效果是如果没有数据到达时（到达网卡并拷贝到了内核缓冲区），立刻返回一个错误值（-1），而不是阻塞地等待。
非阻塞的 read，指的是在数据到达前，即数据还未到达网卡，或者到达网卡但还没有拷贝到内核缓冲区之前，这个阶段是非阻塞的。当数据已到达内核缓冲区，此时调用 read 函数仍然是阻塞的，需要等待数据从内核缓冲区拷贝到用户缓冲区，才能返回。
（2）为每个客户端创建一个线程去调用操作系统的 read 函数

缺点：为每个客户端创建一个线程，服务器端的线程资源很容易被耗光。

3.可以每 accept 一个客户端连接后，将这个文件描述符（connfd）放到一个数组里, 然后弄一个新的线程去不断遍历这个数组，调用每一个元素的非阻塞 read 方法。这样，我们就成功用一个线程处理了多个客户端连接。

缺点：每次遍历遇到 read 返回 -1 时仍然是一次浪费资源的系统调用。所以，还是得恳请操作系统老大，提供给我们一个有这样效果的函数，我们将一批文件描述符通过一次系统调用传给内核，由内核层去遍历，然后告诉我们哪些文件描述符是准备就绪的。

4. [IO 多路复用-select]，操作系统提供了 select 函数，通过它，我们可以把一个文件描述符的数组发给操作系统，让操作系统去遍历，在那些可以读写的文件描述符做上标记然后返回给我们。
当 select 函数返回后，用户依然需要遍历刚刚提交给操作系统的 list。我们需要过滤出有标记的文件描述符去遍历调用 read 函数，这样用户层将不会再有无意义的系统调用开销。
...
5. [IO 多路复用-poll]，poll 也是操作系统提供的系统调用函数。它和 select 的主要区别就是，去掉了 select 只能监听 1024 个文件描述符的限制。

缺点同上

6. [IO 多路复用-epoll]，epoll 是最终的大 boss，它解决了 select 和 poll 的一些问题。
